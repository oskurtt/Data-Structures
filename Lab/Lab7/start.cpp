...
int num_paths(int x, int y, std::vector<std::vector<GRID_STATUS>> blocked_grid){
  if (blocked_grid[x][y] == GRID_BLOCKED){
      ...
      return 1;
        ...
        return num_paths(x, y-1,blocked_grid);
    }
    else if (y == 0 && x>0){
        return num_paths(x-1,y,blocked_grid);
    }
    else{
        return num_paths(x-1,y,blocked_grid)+num_paths(x, y-1,blocked_grid);
    }
    return 0;
}

bool check3(const int& x, const int& y, std::vector<std::vector<GRID_STATUS>>& grid) {
  if (x == 0 && y == 0) {
    grid[x][y] = PATH;
    return true;
  }
  if (x > 0 && grid[x-1][y] != GRID_BLOCKED) {
    if (check3(x-1,y, grid)) {
      grid[x][y] = PATH;
      return true;
    }
  }
  if (y > 0 && grid[x][y-1] != GRID_BLOCKED) {
    if (check3(x, y-1,grid)) {
      grid[x][y] = PATH;
      return true;
    }
  }
...
bool find_path(int x, int y, std::vector<std::vector<GRID_STATUS>> grid){
  if (grid[x][y] == GRID_BLOCKED){
      ...
      grid[x][y] = PATH;
      find_path(x, y-1,grid);
      return true;
    }
    else if (y == 0 && x>0){
      grid[x][y] = PATH;
      find_path(x-1, y,grid);
      return true;
    }
    else{
        return find_path(x-1,y,grid)+num_paths(x, y-1,grid);
    }
    return false;
}
...
int main(int argc, char* argv[]) {
  ...
  //std::cout<<num_paths(start_x, start_y, blocked_grid)<<std::endl;
  check3(start_x, start_y,blocked_grid);
  print_grid(blocked_grid, start_x, start_y);
  return 0;
}
