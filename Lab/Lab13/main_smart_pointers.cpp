...
void deleteAll_rec(Balloon* b, std::set<Balloon*>& nodes);
// ====================================================
// ====================================================
void deleteAll_rec (Balloon* b, std::set<Balloon*>& nodes){
  if (b == NULL) return;
        
  for (int c = 0; c < b->numRopes(); c++) {
    Balloon* next = b->getRope(c); 
    if (nodes.find(next) != nodes.end()) continue; 
    nodes.insert(next); 
    deleteAll_rec(next, nodes); 
  } 
}
void deleteAll(Balloon* b){
  std::set<Balloon*> nodes;
  deleteAll_rec(b, nodes); 
