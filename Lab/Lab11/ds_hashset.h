...
public:
  ...
  std::pair< iterator, bool > insert(KeyType const& key) {
    ...
    iterator p = find(key);
    if (p != end()) return std::make_pair(p, false);
  ...
  iterator begin() {
    for (int i = 0; i < this->m_table.size(); i++){
      if (this->m_table[i].size() < 0){
        return iterator(this, i, this->m_table[i].begin());
      }
    }
    return end();
  }
  ...
  void resize_table(unsigned int new_size) {
    ...
    std::vector<KeyType> data;  // Stores all keys that are part of this current set
      ...
      for (int i = 0; i < m_table.size(); i++) {
          if (m_table[i].size() > 0){
            for (typename std::list<KeyType>::iterator it = m_table[i].begin(); it != m_table[i].end(); it++){
              data.push_back(*it);
            }
